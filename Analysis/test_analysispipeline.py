...
class TestBuildCommand:
    def test_ps_with_target_dir(self, tmp_path):
        ...
        assert cmd == [
            "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(tmp_path / "ArchAnalysis" / "archgen_local.ps1"),
    ...
    def test_ps_without_target_dir(self, tmp_path):
        ...
        assert cmd == [
            "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(tmp_path / "ArchAnalysis" / "archxref.ps1")
        ]
    ...
    def test_python(self, tmp_path):
        ...
        cmd = build_command(step, None, tmp_path)
        assert cmd == [
            sys.executable, str(tmp_path / "ArchAnalysis" / "arch_overview_local.py")
        ]
    ...
    def test_args_appended(self, tmp_path):
        ...
        cmd = build_command(step, None, tmp_path)
        assert cmd == [
            "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(tmp_path / "ArchAnalysis" / "archpass2_local.ps1"),
            "-Top", "100"
        ]
...
class TestGetRepoRoot:
    def test_normal(self, tmp_path):
        # Create fake directory structure: tmp/ArchAnalysis/fake_script.py + tmp/.env
        arch_dir = tmp_path / "ArchAnalysis"
        arch_dir.mkdir()
        (tmp_path / ".env").touch()
