...
Complete technical reference for the architecture documentation pipeline using a local Ollama LLM server with optional clangd LSP. All scripts live in `ArchAnalysis/` and are run from the repo root.
...
Designed for consumer GPU hardware (RTX 3090) with models like `qwen2.5-coder:14b` (~32K token context).
...
| LLM Server       | Ollama (`LLM_HOST:LLM_PORT`, default `192.168.1.126:11434`)    |
| Model            | `qwen2.5-coder:14b` (configurable)                             |
| Context window   | ~32K tokens                                                     |
| Target codebase  | C&C Remastered Collection source release (TD 1995 + RA 1996 C++ DLL source + C# map editor) |
| Toolkit location | `ArchAnalysis/` subdirectory                                                                |
| Config           | `.env` in `ArchAnalysis/` (alongside the scripts)                                           |
| Output           | `architecture/` at repo root                                    |
...
|--------------|----------------------------------------------|------------|----------------------------------------|
| LSP Extract  | `.\ArchAnalysis\serena_extract.ps1`          | 0 (free)   | Symbol data via clangd (optional)      |
| Pass 1       | `.\ArchAnalysis\archgen_local.ps1`           | 1 per file | Per-file architecture docs             |
| Cross-ref    | `.\ArchAnalysis\archxref.ps1`                | 0 (free)   | Function mappings, call graph edges    |
| Graphs       | `.\ArchAnalysis\archgraph.ps1`               | 0 (free)   | Mermaid call graph + subsystem diagrams|
| Overview     | `.\ArchAnalysis\arch_overview_local.ps1`     | N+1        | Subsystem architecture overview        |
| P2 Context   | `.\ArchAnalysis\archpass2_context.ps1`       | 0 (free)   | Targeted context per file              |
| Pass 2       | `.\ArchAnalysis\archpass2_local.ps1`         | 1 per file | Selective re-analysis                  |
...
- `.env` read from `$PSScriptRoot` (`ArchAnalysis/`, same directory as the scripts)
...
When `ArchAnalysis/.serena_context/` exists:
...
The `#Subsections begin` / `#Subsections end` block lists subdirectories for `ArchPipeline.py`. Comment lines (e.g. `# 530 files`) are ignored by the parser.
...
1. **14B models produce good per-file docs with aggressive context management.** Cap source at 800 lines, simplified prompts, compressed LSP. Measured ~0.11 files/sec on an RTX 3090 with `qwen2.5-coder:14b`.
