...
Complete command-line reference for every script. All scripts live in `ArchAnalysis/` and are run from the repo root.
...
```
Step 0 (free):   .\ArchAnalysis\serena_extract.ps1          LSP symbols (optional)
Step 1:          .\ArchAnalysis\archgen_local.ps1            Per-file .md docs
Step 2 (free):   .\ArchAnalysis\archxref.ps1                Cross-reference index
Step 3 (free):   .\ArchAnalysis\archgraph.ps1               Mermaid diagrams
Step 4:          .\ArchAnalysis\arch_overview_local.ps1      Architecture overview
Step 4b (free):  .\ArchAnalysis\archpass2_context.ps1        Targeted context
Step 5:          .\ArchAnalysis\archpass2_local.ps1          Pass 2 enrichment (selective)
```
...
```
.\ArchAnalysis\archgen_local.ps1
...
.\ArchAnalysis\archgen_local.ps1 -Clean
```

### Performance

~0.11 files/sec (~9s per file) with `qwen2.5-coder:14b` on RTX 3090.
...
```
.\ArchAnalysis\archxref.ps1 [-TargetDir <string>] [-EnvFile <string>]
```
...
```
.\ArchAnalysis\archpass2_context.ps1 [-TargetDir <string>] [-EnvFile <string>]
```
...
```
.\ArchAnalysis\archpass2_local.ps1
...
.\ArchAnalysis\archpass2_local.ps1 -Top 50 -ScoreOnly
.\ArchAnalysis\archpass2_local.ps1 -Only "REDALERT/CONQUER.CPP"
```
...
```
.\ArchAnalysis\serena_extract.ps1
...
```
python ArchAnalysis\generate_compile_commands.py [--root <path>] [--output <path>]
```
...
The `.env` file lives in `ArchAnalysis/` alongside the scripts (not at the repo root). Scripts read it via `$PSScriptRoot`.

### LLM Server

| Variable          | Default              | Description                |
|-------------------|----------------------|----------------------------|
| `LLM_HOST`        | `192.168.1.126`      | Ollama server IP/hostname  |
...
The `.env` file can contain a `#Subsections begin` / `#Subsections end` block listing subdirectories for `ArchPipeline.py` to process in sequence. Comment lines (starting with `#`) are ignored by the parser and can be used for annotations like file counts.
...
Subsections are processed by `ArchPipeline.py` via `-TargetDir <name>`. Scans are recursive, so nested dirs (e.g. `REDALERT/WIN32LIB/`) are picked up inside their parent subsection.
...
```powershell
.\ArchAnalysis\archgen_local.ps1 -Preset cnc    # skips completed files
```
...
```powershell
.\ArchAnalysis\archgen_local.ps1 -Clean
.\ArchAnalysis\archpass2_local.ps1 -Clean
```
