...
Generate architecture documentation for game engine codebases using a local Ollama LLM + optional clangd LSP. All scripts live in `ArchAnalysis/` and are run from the repo root.

---

## Setup

### 1. Configure `ArchAnalysis/.env`
...
```powershell
.\ArchAnalysis\serena_extract.ps1 -Preset cnc       # First run is slower (builds index)
.\ArchAnalysis\archgen_local.ps1 -Preset cnc        # Auto-injects LSP symbols
```
...
```
0 (free)   .\ArchAnalysis\serena_extract.ps1          LSP symbols (optional)
1          .\ArchAnalysis\archgen_local.ps1            Per-file .md docs
2 (free)   .\ArchAnalysis\archxref.ps1                Cross-reference index
3 (free)   .\ArchAnalysis\archgraph.ps1               Mermaid diagrams
4          .\ArchAnalysis\arch_overview_local.ps1      Architecture overview
4b (free)  .\ArchAnalysis\archpass2_context.ps1        Targeted context
5          .\ArchAnalysis\archpass2_local.ps1          Pass 2 enrichment (selective)
```
